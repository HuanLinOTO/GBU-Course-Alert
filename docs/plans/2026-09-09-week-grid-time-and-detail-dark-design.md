...
> **修订（2026-09-09 真机核对后，用户确认）**：比例分母从「本节时长」改为「整行跨度」
> （本节开始 → 下一节开始；末节为 → 本节结束）。课间不再折叠到前一节末尾（1f），
> 而是按真实时间落在行内——12:15 结束的课块底端画在第 6 行 35/50≈0.7 处，
> 不再贴 12:30 刻度线；节内插值同理更忠实（如 14:30 → 第 9 行 30/40=0.75）。
